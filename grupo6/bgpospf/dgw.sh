...
docker exec -it nginx_web_1 apt-get -qy install iproute
...
docker exec -it webminbind_bind_1 apt-get install -qy iproute
...
docker exec -it squid_Squid_1 ip -6 r add default via 2001:b:3::3
